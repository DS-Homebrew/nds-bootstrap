...
static void resetMemory_ARM7(void) {
	...
	*(vu32*)(0x04000000 - 8) = ~0; // VBLANK_INTR_WAIT_FLAGS, ARM7 version
	...
	useTwlCfg = ((*(u8*)0x02000400 & BIT(0) & BIT(1) & BIT(2)) && (*(u8*)0x02000401 == 0) && (*(u8*)0x02000402 == 0) && (*(u8*)0x02000404 == 0));
...
int arm7_main(void) {
	...
	if (logging) {
		enableDebug(getBootFileCluster("NDSBTSRP.LOG", !sdRead));
	}
	...
	*savFile = getFileFromCluster(saveFileCluster);
	...
	if (!gameOnFlashcard && isDSiWare && (REG_SCFG_EXT != 0 || oldArm7mbk != 0x00403000)) {
		...
		*(u16*)0x4000500 = 0x807F;
		...
		} else if ((consoleModel > 0 && (!gameOnFlashcard || !ROMsupportsDsiMode(ndsHeader) || !dsiModeConfirmed))
