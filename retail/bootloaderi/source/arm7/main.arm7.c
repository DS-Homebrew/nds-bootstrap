...
static bool isROMLoadableInRAM(const tDSiHeader* dsiHeader, const tNDSHeader* ndsHeader, const char* romTid, const module_params_t* moduleParams) {
	...
	|| (strncmp(romTid, "APD", 3) != 0
	 ...
	 && strncmp(romTid, "KPF", 3) != 0)
		...
		res = ((dsiModeConfirmed && consoleModel>0 && ((ROMsupportsDsiMode(ndsHeader) && dsiModeConfirmed) ? getIRomSizeNoArmBins(dsiHeader)+ioverlaysSize : getRomSizeNoArmBins(ndsHeader)) < 0x01000000)
		...
		*(u16*)0x4000500 = 0x807F;
		...
		if (isSdk5(moduleParams)) {
			...
			if (ROMinRAM) {
				...
				if (((u32)ndsHeader->arm9destination != 0x02004000 && extendedMemoryConfirmed) || (ROMsupportsDsiMode(ndsHeader) && dsiModeConfirmed)) {
			...
			ce9Location = strncmp(romTid, "ADM", 3)==0||strncmp(romTid, "A62", 3)==0 ? CARDENGINEI_ARM9_CACHED_END_LOCATION : CARDENGINEI_ARM9_CACHED_MID_LOCATION;
