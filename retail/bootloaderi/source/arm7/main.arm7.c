...
int arm7_main(void) {
	...
	*savFile = getFileFromCluster(saveFileCluster);
	...
	if (isDSiWare) {
		...
		tonccpy((u32*)ce7Location, (u32*)(useSdk5ce7 ? CARDENGINEI_ARM7_SDK5_BUFFERED_LOCATION : CARDENGINEI_ARM7_BUFFERED_LOCATION), 0x12800);
		if (gameOnFlashcard || saveOnFlashcard) {
		...
		if (isSdk5(moduleParams)) {
			...
			if (extendedMemoryConfirmed) {
				ce9Location = (((u32)ndsHeader->arm9destination == 0x02004000) ? CARDENGINEI_ARM9_CACHED_LOCATION1 : CARDENGINEI_ARM9_CACHED_LOCATION_ROMINRAM);
				...
				tonccpy((u32*)CARDENGINEI_ARM9_CACHED_LOCATION, (u32*)CARDENGINEI_ARM9_SDK5_DLDI_BUFFERED_LOCATION, 0x7000);
				if (gameOnFlashcard && !dldiPatchBinary((data_t*)ce9Location, 0x7000)) {
