...
void patchSleepInputWrite(cardengineArm7* ce7, const tNDSHeader* ndsHeader, const module_params_t* moduleParams, const bool buttonsRemapped) {
...
bool a7PatchCardIrqEnable(cardengineArm7* ce7, const tNDSHeader* ndsHeader, const module_params_t* moduleParams) {
	...
	if (usesThumb) {
		...
		tonccpy(cardIrqEnableOffset, cardIrqEnablePatch, 0x14);
	} else {
...
u32 patchCardNdsArm7(
	...
	if (ndsHeader->unitCode > 0 && ((u32)ndsHeader->arm9destination+ndsHeader->arm9binarySize) < DONOR_ROM_ARM7_LOCATION && *(u32*)DONOR_ROM_ARM7_SIZE_LOCATION != 0) {
