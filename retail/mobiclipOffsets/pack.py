...
parser = argparse.ArgumentParser(description="Packs many small files into a single file for nds-bootstrap")
...
with open(args.output[0], "wb") as f:
	...
	for file in files:
		...
		# Header CRC-16, Position of data, Size of data, Version number, Padding
		f.write(struct.pack("<Hllbx", int(file["name"][5:file["name"].find(".")], 16), position, len(file["data"]), -1 if file["name"][-3:] == "bin" else 1 if file["name"][-2:] == "01" else 0))
