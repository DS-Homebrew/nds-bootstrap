	...
	if (boot) {
		...
		for (int i = 0; i < 256; i++) {
			...
			*(u8*)(0x02000838+i2) = *(u8*)(ce7+0x8400+i);		// Unlaunch Device:/Path/Filename.ext (16bit Unicode,end by 0000h)
			#else
			*(u8*)(0x02000838+i2) = *(u8*)(ce7+0xF000+i);	// Unlaunch Device:/Path/Filename.ext (16bit Unicode,end by 0000h)
			#endif
			i2 += 2;
		}
	}
...
static void readSrBackendId(void) {
	...
	*(u32*)(0x02000318) = /* *(u32*)(ce7+0x8504) == 0x00030000 ? 0x13 : */ 0x17;
	#else
	*(u32*)(0x02000310) = *(u32*)(ce7+0xF100);
	*(u32*)(0x02000314) = *(u32*)(ce7+0xF104);
	*(u32*)(0x02000318) = /* *(u32*)(ce7+0xF104) == 0x00030000 ? 0x13 : */ 0x17;
	#endif
	*(u32*)(0x0200031C) = 0;
...
void reset(void) {
	...
	if (((valueBits & isDlp) && *(u32*)(NDS_HEADER_SDK5+0xC) == 0) || (valueBits & slowSoftReset) || (*(u32*)(resetParam+0xC) > 0 && (*(u32*)CARDENGINEI_ARM9_LOCATION == 0 || (valueBits & isSdk5)))) {
		...
		*(u32*)0x02000004 = 0x54455352; // 'RSET'
		if (consoleModel < 2) {
			(*(u32*)(ce7+0xF100) == 0 && (valueBits & b_dsiSD)) ? unlaunchSetFilename(false) : unlaunchSetHiyaFilename();
		}
		if (*(u32*)(ce7+0xF100) == 0 && (valueBits & b_dsiSD)) {
	...
	*(u32*)0x02000004 = 0x54455352; // 'RSET'
	...
	if (consoleModel < 2) {
		if (valueBits & b_dsiSD) {
			...
			#else
			(*(u32*)(ce7+0xF100) == 0) ? unlaunchSetFilename(false) : unlaunchSetHiyaFilename();
			#endif
		}
...
void returnToLoader(bool reboot) {
	...
	*ndsHeader = __DSiHeader->ndshdr;
	...
	if (consoleModel >= 2) {
		...
		else if (*(char*)(ce7+0xF103) == 'H' || *(char*)(ce7+0xF103) == 'K')
		{
			// Use different SR backend ID
			readSrBackendId();
		}
