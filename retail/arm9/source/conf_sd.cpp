...
int loadFromSD(configuration* conf, const char *bootstrapPath) {
  ...
  if (conf->gameOnFlashcard || !conf->isDSiWare) {
	...
	if (b4dsDebugRam) {
		...
		if ((accessControl & BIT(4)) && strncmp(romTid, "K2Z", 3) != 0 && strncmp(romTid, "KUG", 3) != 0 && strncmp(romTid, "KBG", 3) != 0 && strncmp(romTid, "Z2E", 3) != 0 && strncmp(romTid, "K59", 3) != 0 && strncmp(romTid, "KG4", 3) != 0) {
		// If it has access to TWLNAND (or uses dataPub/dataPrv),
		// and the title isn't "G.G Series: Altered Weapon", "G.G Series: Drift Circuit 2", "Chotto DS Bun ga Kuzenshuu: Sekai no Bungaku 20", "Famicom Wars DS: Ushinawareta Hikari", "Metal Torrent", or "Saikyou Ginsei Shougi"...
