...
u32* patchLoHeapPointer(const module_params_t* moduleParams, const tNDSHeader* ndsHeader, bool ROMinRAM) {
	...
	if (ROMinRAM) {
		...
		*heapPointer += (isSdk5(moduleParams) ? 0x3000 : 0x2800); // shrink heap by 10KB (or for SDK5, 12KB)
	}
...
u32* patchHiHeapPointer(const module_params_t* moduleParams, const tNDSHeader* ndsHeader, bool ROMinRAM) {
	...
	if (ROMinRAM) {
		...
		*heapPointer = (gameOnFlashcard ? CARDENGINE_ARM9_DLDI_LOCATION : CARDENGINE_ARM9_CACHED_LOCATION); // shrink heap by 16KB or 10KB
	}
