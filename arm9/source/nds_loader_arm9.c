...
int runNds (const void* loader, u32 loaderSize, u32 cluster, u32 saveCluster, u32 donorCluster, u32  patchMpuRegion, u32 patchMpuSize, bool initDisc, bool dldiPatchNds, u32 forceDecompression, int argc, const char** argv)
{
	char* argStart;
	u16* argData;
	u16 argTempVal = 0;
	...
	*argData = argTempVal;
	...
	writeAddr ((data_t*) LCDC_BANK_C, DECOMP_OFFSET, forceDecompression);
...
int runNdsFile (const char* filename, const char* savename,  const char* arm7DonorPath, int patchMpuRegion, int patchMpuSize,u32 forceDecompression, int argc, const char** argv)  {
	struct stat st;
	struct stat stSav;
	struct stat stDonor;
	u32 clusterSav = 0;
	...
	return runNds (load_bin, load_bin_size, st.st_ino, clusterSav, clusterDonor, patchMpuRegion, patchMpuSize, true, true, forceDecompression, argc, argv);
}
