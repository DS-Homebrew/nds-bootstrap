...
#define CARDENGINE_ARM7_OFFSET 72
#define CARDENGINE_ARM9_OFFSET 76
...
int runNds (const void* loader, u32 loaderSize, u32 cluster, u32 saveCluster, u32 saveSize, u32 donorSdkVer, u32 patchMpuRegion, u32 patchMpuSize, u32 consoleModel, u32 ntrTouch, u32 loadingScreen, u32 romread_LED, u32 gameSoftReset, bool initDisc, bool dldiPatchNds, int argc, const char** argv, u32* cheat_data)
{
	char* argStart;
	u16* argData;
	u16 argTempVal = 0;
...
int runNdsFile (const char* filename, const char* savename, int saveSize, int donorSdkVer, int patchMpuRegion, int patchMpuSize, int consoleModel, int ntrTouch, int loadingScreen, int romread_LED, int gameSoftReset, int argc, const char** argv, u32* cheat_data)  {
	struct stat st;
	struct stat stSav;
	u32 clusterSav = 0;
	...
	return runNds (load_bin, load_bin_size, st.st_ino, clusterSav, saveSize, donorSdkVer, patchMpuRegion, patchMpuSize, consoleModel, ntrTouch, loadingScreen, romread_LED, gameSoftReset, true, true, argc, argv, cheat_data);
}
...
int loadCheatData (u32* cheat_data) {
    ...
    u32 cheatDataOffset = cardengineArm7[11];
